...
**Resolved during real-machine Task 10 testing (2026-07-05) — two attempts before landing on the working mechanism:**

1. **Original design** (self-quit immediately after `Process.Start`, rely on Inno Setup's `/RESTARTAPPLICATIONS`) — the app quit before Setup's Restart Manager integration ever got a chance to register it as something to restart. Silent install succeeded (files replaced correctly), but the app never came back.
2. **First fix attempt** (stop calling `Application.Quit()`, let `/CLOSEAPPLICATIONS` + Restart Manager detect and close the still-running process) — **this hung indefinitely.** Unity standalone builds don't call `RegisterApplicationRestart` or handle `WM_QUERYENDSESSION`, so Windows Restart Manager can never actually ask them to close gracefully; Setup just waited forever with the app still running and no files replaced. Confirmed by watching process state directly (`RealmWeaver.exe` and two `CodeSetup-stable-*` processes all still alive 30+ seconds later, install directory untouched).
3. **Working fix:** restore the original self-quit (right after `Process.Start`, since that part was never broken), but replace `/RESTARTAPPLICATIONS` — which depends on Restart Manager cooperation this app doesn't have — with the installer's own `[Run]` section (`installer/RealmWeaver.iss`), which relaunches the app unconditionally after install completes, once its `skipifsilent` flag is removed. `CloseApplications`/`RestartApplications` were removed from `[Setup]` entirely since they do nothing useful here. Verified end-to-end on a real install: v0.0.6 → v0.0.7 self-update closed and relaunched successfully with no manual intervention.

The lesson for any future change to this flow: **the fix only takes effect once the version containing it is the one actually running and driving the next update** — testing a fix requires manually installing the fixed version first (not via its own broken update mechanism), then releasing one more version to exercise the fix for real.
