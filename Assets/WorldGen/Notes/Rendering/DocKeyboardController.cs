...
namespace WorldGen.Notes.Rendering
{
    ...
    public class DocKeyboardController : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            if (active == null)
            {
                // ПОПАП «@» ЗАКРЫВАЕТСЯ ЗДЕСЬ, и это не уборка «на всякий случай» (задача 6). Раньше этот
                // ранний return означал «активная панель показывает карту» — состояние, в которое из
                // открытого попапа не попасть. Теперь он означает ещё и «каретка ушла в развёрнутую доску»,
                // а туда ДМ попадает одним кликом из строки с открытым списком. Своего перехватчика кликов
                // у попапа нет намеренно (см. его класс-док, «БЕЗ BACKDROP'А»), закрывает его
                // UpdateMentionPopup — до которого этот return как раз и не даёт дойти, — а AdoptView
                // срабатывает только при смене вида на ДРУГОЙ, но не на «никакой». Без этой строки список
                // подсказок висел бы над панелью, которую ДМ уже покинул. Тот же довод, что и у AdoptView.
