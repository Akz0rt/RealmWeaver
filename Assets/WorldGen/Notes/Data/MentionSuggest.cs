...
namespace WorldGen.Notes.Data
{
    ...
    /// suggestions are the page's OWN mentions, most-recently-mentioned first (ParseSpans over every block's
    /// Text and Detail, exactly how NotesDocOps.FindBacklinks decides what counts as a mention — a mention
    /// hidden in a ⊕ Detail body is a mention there and it is one here too), then whatever the DM inserted
    /// via this very popup earlier in the session (`recentIds`, order preserved, nothing re-ranks it).
    ///
    ...
    public static class MentionSuggest
    {
            ...
            IReadOnlyList<WorldObjectRef> world, string query, IReadOnlyList<string> recentIds, int limit)
        {
            ...
            if (needle.Length == 0)
            {
                ...
                foreach (var m in mentioned)
                {
                    ...
                    if (!seen.Add((m.Kind, m.Id))) continue;   // не должно случиться — MentionedOnPage уже без повторов
                    result.Add(m);
                }
        ...
        /// <summary>Rule 2's ordering: every well-formed link inside `current`'s own blocks (Text AND Detail
        /// of each — the exact fields NotesDocOps.BlockReferencesPage reads for «Упомянута в», found the
        /// exact same way, via ParseSpans), one entry per distinct (kind,id), ordered by the position of its
        /// LAST occurrence, most-recent first. A self-link is dropped inside the walk itself (rule 6), and a
        /// span whose target no longer exists resolves to null and is silently skipped — there is nothing
        /// to show a name for.</summary>
