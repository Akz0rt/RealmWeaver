...
namespace WorldGen.Notes.Data
{
    ...
    public static class StrokeOps
    {
        ...
        /// <summary>Ограничитель работы: без него точка в вырожденной цепочке (все точки ближе
        /// CornerArmMinLength друг к другу) уходила бы до конца мазка, давая O(n²).
        ///
        /// После Dedupe шестнадцать шагов покрывают не меньше 0.032 ДЛИНЫ ПУТИ — вдвое больше, чем
        /// требуется плечу. Но плечо меряется ХОРДОЙ, а не путём, поэтому потолок всё-таки может
        /// сработать: на тугом завитке (шестнадцать точек по 0.002 вокруг круга радиусом 0.005 —
        /// это полный оборот) хорда почти нулевая. Тогда TryArm вернёт false и точка просто не
        /// будет классифицирована — тот же осторожный исход, что и у концов мазка, и на таком
        /// масштабе он верен: завиток размером с саму дрожь сломом не является.
        ///
        /// Мутанта на это число нет: любое значение, при котором потолок вообще достижим, меняет
        /// поведение лишь на завитках, которые ни одна проверка не описывает и описывать не
        /// должна.</summary>
