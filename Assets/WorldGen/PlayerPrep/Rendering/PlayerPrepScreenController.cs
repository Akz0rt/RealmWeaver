...
namespace WorldGen.PlayerPrep.Rendering
{
    ...
    public class PlayerPrepScreenController : MonoBehaviour
    {
        ...
        /// кто ею потом воспользуется.
        ///
        /// ЛОВУШКА ДЛЯ ЗАДАЧИ 12: обращаться сюда только через `if (Sheet != null)`, а НЕ через
        /// `Sheet?.Refresh()`. Тип наследует UnityEngine.Object, у которого сравнение с null
        /// перегружено и знает про уничтоженные объекты; оператор `?.` этой перегрузкой не
        /// пользуется — он проверяет настоящую ссылку и на уничтоженном листе пропустит вызов
        ...
        /// над УЖЕ сохранённым листом, передавайте OpenWizard(file, CurrentPath).
        ///
        /// ⚠ ВХОД ПУБЛИЧНЫЙ, А КОНТРАКТ — НЕТ. «Создать ещё одного» — это OpenWizard(new CharacterFile
        /// { … }) И СРАЗУ RememberSaved(), как делает CreateNew; второй половины снаружи не достать,
        /// метод приватный. Позови кто-нибудь один только OpenWizard — новый пустой персонаж унаследует
        /// снимок ПРЕЖНЕГО, и выход с него спросит «сохранить?» там, где терять нечего. Цена — лишний
        /// вопрос, а не потерянная работа, поэтому это оставлено как есть; понадобится звать извне —
        /// снимайте снимок здесь, а из CreateNew уберите.</summary>
