...
namespace WorldGen.PlayerPrep.Rendering
{
    ...
    public class WizardView : MonoBehaviour
    {
        ...
        /// они занимали бы ячейки класса, оставаясь при этом серыми надписями без кнопки.
        ///
        /// КОМПЕТЕНТНОСТИ ПРИ ЭТОМ НЕ КАСАЕТСЯ НИКТО, и это нарочно (см. ApplyBackgroundChange, где
        /// сказано подробно). Строка, снимавшая «осиротевшую» компетентность, снимала её как раз с
        /// тех навыков, которые новая предыстория даёт даром: владение остаётся, а компетентность
        /// исчезала. Обратный случай — навык, которого новая предыстория больше не даёт, — чинится
        /// сам при чтении: SheetMath.ExpertiseChosenIds пропускает только компетентность на навыке,
        /// которым персонаж владеет.
        ///
        /// Снаряжение пересобирается тут же и по той же причине, что при смене класса, —
        /// см. ApplyClassAndKit.</summary>
        ...
        void BuildStepSkills(Transform content)
        {
            ...
            foreach (var option in WizardOps.AvailableSkills(file, rules))
            {
                ...
                {
                    // Снимается ТОЛЬКО владение. Компетентность на этом навыке в файле остаётся, и
                    // трогать её здесь нельзя: снятая галочка — это часто промах мимо кнопки, а
                    // стёртая компетентность обратно с галочкой не возвращается. Работать она всё
                    // равно перестаёт — SheetMath.ExpertiseChosenIds пропускает только компетентность
                    // на навыке, которым персонаж владеет, — и по тому же фильтру считает счётчик
                    // «выбрано N из M» ниже, так что он не залипает и строка из списка уходит. Ровно
                    // так же в этой арке устроены понижение уровня и смена предыстории: правило живёт
                    // в чистом слое ПРИ ЧТЕНИИ, а в файле строка лежит безвредно. Это была последняя
                    // правка-по-правилу в слое рисования.
                    if (chosen) file.SkillIds.Remove(o.Id);
        ...
        void BuildExpertise(Transform content, ClassDef cls)
        {
            ...
            // СЧИТАЕМ ТО ЖЕ, ЧТО И ЛИСТ, И ТОЙ ЖЕ ФУНКЦИЕЙ: SheetMath.ExpertiseChosenIds. Разойдись
            // счёт — старый файл попадал бы в тупик: лист говорит «выбрано 0 из 1», а мастер считает
            // 1 из 1, гасит все кнопки и не рисует строку, которой этот единственный счёт занят, —
            // снять её нечем, поставить новую тоже.
            //
            // Потолок по уровню тут НЕ применяется нарочно (см. ExpertiseChosenIds): Барду,
            // опустившемуся с 9 уровня на 8, честнее сказать «выбрано 4 из 2» и показать, сколько
            // строк снять, чем написать «2 из 2» при четырёх отмеченных.
            int chosenCount = SheetMath.ExpertiseChosenIds(file, rules, cls).Count;

            AddSpacer(content, 12f);
            AddCaption(content, $"Компетентность: выбрано {chosenCount} из {allowed}");
            AddLabel(content, "Компетентность удваивает бонус мастерства. Выбирается из навыков, "
                              + "которыми персонаж владеет, — и взятых у класса, и данных "
                              + "предысторией.", 15, Muted, null);

            // ВЛАДЕНИЕ ЦЕЛИКОМ, а не один file.SkillIds: правила говорят «два навыка, которыми ты
            // владеешь», не спрашивая, откуда владение пришло, и Волшебник-Мудрец обязан видеть тут
            // Магию с Историей, полученные от предыстории. Решает SheetMath.ExpertiseEligibleIds —
            // одна функция на мастер, лист и смену класса.
            //
            ...
            var eligible = SheetMath.ExpertiseEligibleIds(file, rules, cls);
            ...
            if (eligible.Count == 0)
            {
                // Одна строка на два случая: навыков нет вовсе и ни один из взятых классу не
                // подходит. Ответ игроку в обоих одинаков — взять подходящий навык выше.
                AddLabel(content, "Ни одного подходящего навыка у персонажа ещё нет — выберите "
                                  + "такой выше.", 16, Faint, null);
                return;
            }
