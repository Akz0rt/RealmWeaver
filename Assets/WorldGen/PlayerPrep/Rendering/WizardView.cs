            ...
            foreach (var option in WizardOps.AvailableSkills(file, rules))
            {
                ...
                var btn = AddButton(row, $"{(chosen ? "[×]" : "[ ]")} {o.Name}", 320f, () =>
                {
                    if (chosen)
                    {
                        ...
                        // Компетентность держится на ВЛАДЕНИИ навыком, а этот навык персонаж только
                        // что снял — здесь, в строке класса, он есть только потому, что предыстория
                        // его не даёт (иначе строка была бы серой «уже есть»). Значит владения не
                        // осталось, и компетентность уходит вместе с ним; лист посчитал бы так же
                        // (SheetMath.ExpertiseChosenIds), но снять её было бы уже нечем — строки
                        // компетентности на невзятый навык мастер не рисует.
