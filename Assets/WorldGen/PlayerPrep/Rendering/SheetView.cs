...
namespace WorldGen.PlayerPrep.Rendering
{
    ...
    public class SheetView : MonoBehaviour
    {
        ...
        void BuildBottomBar()
        {
            ...
            // Кнопка гаснет там, где нажатие ничего не даст, — включённая кнопка, которая ничего не
            // делает, читается как поломка. Случаев три, и все три настоящие:
            //   20 уровень            — выше LevelPlanOps.LevelUp не пускает;
            //   класса нет или он чужой — LevelUp не трогает файл вовсе, чтобы не поднять уровень
            //                             по таблице, которой нет; кнопка выглядела бы сломанной;
            //   уровень в файле меньше единицы — испорченный файл, где лист честно показывает 1
            //                             (прижатое), а нажатие двигало бы −3 на −2 у всех на глазах.
            // Ни одно из трёх не считает правило заново: LevelUp по-прежнему проверяет всё сам.
            bool canRaise = live && sheet.Level < 20 && CurrentClass() != null && file.Level >= 1;
            AddBarButton(bar, "Повысить уровень", 454f, 230f, RaiseLevel, canRaise);
        ...
        /// вторую дорогу, уровень поднимался бы молча.
        ///
        /// ПАНЕЛЬ ОТКРЫВАЕТСЯ, ТОЛЬКО ЕСЛИ ЕЙ ЕСТЬ ЧТО ПОКАЗАТЬ. LevelUp возвращает шаги трёх
        /// видов, а панель плана умеет два — подкласс и повышение характеристик; про компетентность
        /// она сказать не может ничего (у строки уровня ChoiceKind остаётся пустым, и внутри
        /// показывать нечего). Раньше третий вид был недостижим: у единственного класса с
        /// компетентностью, Плута, она на 1 уровне, а LevelUp сравнивает со СЛЕДУЮЩИМ — то есть с
        /// двойкой и выше. Теперь у Барда, Волшебника и Следопыта компетентность на 2 уровне, и
        /// Бард 1 уровня по кнопке получал пустую панель, в которой нечего выбрать.
        ///
        /// Дорога к выбору компетентности прежняя и другая: строка «Чего не хватает» на листе плюс
        /// кнопка «Изменить», ведущая в мастер. Отдельного экрана для неё нет намеренно.</summary>
        void RaiseLevel()
        {
            if (file == null || rules == null) return;
            var steps = LevelPlanOps.LevelUp(file, rules);
            LevelChoiceOps.ApplyPlanFor(file, file.Level);
            if (steps.Any(s => s.Kind == "subclass" || s.Kind == "asi")) OpenPlan(file.Level);
            else Refresh();
        }
        ...
        string Subtitle()
        {
            ...
            //
            // Спрашивается ПОСЧИТАННЫЙ подкласс, а не file.SubclassId. Разница видна после «−»: выбор
            // в файле остаётся (это выбор, а не состояние), но на 2 уровне подкласса у персонажа ещё
            // нет — а подзаголовок печатал «плут 2 · вор». Проверять «дорос ли» здесь нельзя: то же
            // условие уже стоит в чистом слое, где строится «Подкласс не выбран».
            if (!string.IsNullOrEmpty(sheet.ActiveSubclassId))
                parts.Add(Part(SubclassName(), sheet.ActiveSubclassId, "подкласс", ""));
