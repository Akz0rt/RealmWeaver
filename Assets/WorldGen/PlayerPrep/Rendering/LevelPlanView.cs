    ...
    public class LevelPlanView : MonoBehaviour
    {
        ...
        string ChosenText(PlanRow row, ClassDef cls)
        {
            ...
            if (row.ChosenKind == "asi")
            {
                ...
                // Пометка без прибавок — законное состояние, хотя нынешняя панель его больше не
                // создаёт: так выглядят листы, сохранённые ПРЕЖНИМ правилом, которое урезало прибавку
                // потолком в миг записи и на самой двадцатке не писало ничего. Говорим об этом
                // словами, а не пустым местом.
                if (bumps.Count == 0) return "прибавки не записаны";
                // Печатается ЗАПИСАННОЕ — то самое «+2», которое выбрал игрок. Урезать строку потолком
                // здесь нельзя дважды: во-первых, это была бы вторая копия правила, живущего в
                // SheetMath.AbilityTotal; во-вторых, надкусанная прибавка вернётся целиком, если игрок
                // понизит базу в мастере. Сколько из обещанного дошло, говорит лист
                // (SheetMath.ExplainAbility) — там, где стоит само число.
