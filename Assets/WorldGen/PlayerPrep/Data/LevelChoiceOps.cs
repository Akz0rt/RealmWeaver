...
namespace WorldGen.PlayerPrep.Data
{
    ...
    public static class LevelChoiceOps
    {
        ...
        ///
        /// ЗАПИСЫВАЕТСЯ ОБЕЩАННОЕ ЦЕЛИКОМ, А ПОТОЛОК 20 ОТРЕЗАЕТ ПРИ ЧТЕНИИ — тем же приёмом, что
        /// компетентность сверх открытого уровня и компетентность на потерянном навыке (см.
        /// SheetMath.AbilityTotal и SheetMathSkills). Раньше «+2» здесь урезалось до остатка ещё в
        /// миг записи, и панель обещала «Готово: +2 Ловкость», клала в файл +1 и молчала об этом:
        /// ни кнопка, ни строка ячейки, ни лист не говорили, что половина пропала, — а игрок ещё мог
        /// бы выбрать вместо этого +1 в две характеристики.
        ///
        /// Теперь потерю называет вслух лист: SheetMath.ExplainAbility сравнивает ЗАПИСАННОЕ с
        /// дошедшим и пишет «Ловкость 19 → 20 (+1 из +2: выше 20 характеристика не растёт)». Пока
        /// обрезка стояла тут, эта ветка объяснения была по здешней дороге недостижима вовсе —
        /// сравнивать было нечего.
        ///
        /// ВТОРОЙ ВЫИГРЫШ — ПРАВКА ЗАДНИМ ЧИСЛОМ. Игрок, понизивший потом базу характеристики в
        /// мастере (19 → 15), получает своё повышение обратно ПОЛНЫМ: в файле лежит +2, и счёт
        /// выдаёт 17. Обрезанное при записи осталось бы обрезанным навсегда — файл забыл бы, что
        /// выбирал игрок. Чаще всего речь о НАДКУСАННОЙ прибавке: характеристику, уже стоящую на
        /// двадцати, панель выбрать не даёт вовсе (кнопка гаснет), так что этой дорогой сюда не
        /// попасть. Но если такой выбор всё же пришёл — из чужого файла или правкой руками, — правило
        /// одно на всех: обещанное ложится целиком, и режет его тот же счёт.
        ///
        /// СТАРЫЕ ПРИБАВКИ СНИМАЮТСЯ ДО ЗАПИСИ НОВЫХ, и порядок здесь — не стиль: RemoveBumps сносит
        /// ВСЕ прибавки этого уровня, поэтому вызов после записи стёр бы ровно то, что только что
        /// положили, и ячейка осталась бы пустой.</summary>
        public static void ChooseAsi(CharacterFile file, int level, IList<string> abilityIds)
        {
            ...
            foreach (var id in picked)
                ...
                {
                    Source = SheetMath.BumpSource(level), AbilityId = id, Amount = each
                });
        }
