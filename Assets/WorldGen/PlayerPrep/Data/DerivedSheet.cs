...
namespace WorldGen.PlayerPrep.Data
{
    ...
    public class DerivedSheet
    {
        ...
        /// <summary>Подкласс, который на этом уровне ДЕЙСТВУЕТ, либо null. Не то же самое, что
        /// file.SubclassId: у понизившего уровень персонажа выбор в файле остаётся (это его выбор, а не
        /// состояние), но до уровня, на котором класс даёт подкласс, тот ещё не работает.
        ///
        /// Отдаётся наружу затем, чтобы лист не решал это сам. Подзаголовок печатал подкласс по
        /// непустому file.SubclassId и после «−» до 2 уровня читался «плут 2 · вор» — подписи, которой
        /// на этом уровне взяться неоткуда. А своя проверка «дорос ли» в слое рисования была бы второй
        /// копией того же условия, по которому строится «Подкласс не выбран», и разошлась бы с ним.</summary>
        public string ActiveSubclassId;
