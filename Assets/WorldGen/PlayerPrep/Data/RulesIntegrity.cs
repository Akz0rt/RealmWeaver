...
namespace WorldGen.PlayerPrep.Data
{
    ...
    ///
    /// Отдельно проверяется всё, что портит АРИФМЕТИКУ листа, а не просто молчит:
    /// `UnarmoredDefenseAbility` вне шести характеристик отнимает у КД целую характеристику (раньше
    /// он ещё и ронял лист Варвара с IndexOutOfRange — теперь SheetMathSkills спрашивает
    /// SheetMath.AbilityName и печатает незнакомое как есть), `Kind` с опечаткой роняет КД до
    /// 10 + ловкость, а `HitDie` не вида d8 оставляет максимум хитов нулём.
    ///
    /// ЗОВЁТСЯ НЕ ТОЛЬКО В ГЕЙТЕ: PlayerPrepScreenController зовёт Check сразу после разбора
    /// справочника и показывает ошибки на экране списка. Иначе сторож стерёг бы только тот
    /// справочник, что уже прошёл сборку, — а вся его ценность в правке справочника БЕЗ пересборки,
    /// то есть там, где никакого гейта нет.</summary>
    public static class RulesIntegrity
    {
        ...
        /// значение вне шести: КД теряет целую характеристику (модификатор незнакомой равен нулю), а
        /// в объяснении вместо «мудрость» стоит сам идентификатор. Раньше было хуже — SheetMathSkills
        /// искал название через Array.IndexOf и на −1 падал с IndexOutOfRange, то есть лист не
        /// открывался вовсе; теперь он спрашивает SheetMath.AbilityName, и опечатка стала видимой, а
        /// не смертельной. Проверка от этого не лишняя, а наоборот: молчаливую потерю бонуса замечает
        /// только она.</summary>
        ...
        /// SkillChoices. Это правило ПРО ОПЕЧАТКУ В СПРАВОЧНИКЕ, а не про то, что иначе навык был бы
        /// недостижим: владение приходит и от предыстории (см. SheetMath.ProficientSkillIds), так
        /// что компетентность в навыке вне SkillChoices персонаж получить в состоянии. Но у всех
        /// двенадцати классов правил 2024 шестёрка Волшебника — единственное сужение, и каждый её
        /// навык класс же и раздаёт; идентификатор вне SkillChoices здесь почти наверняка описка.
        ///
        ...
        /// законен: он значит «из любого навыка».
        ///
        /// Ослаблять ли правило ради справочника, где класс даёт компетентность в навыке, которым
        /// владеть даёт только предыстория, — вопрос к ДМ, а не к этой починке. Пока такого набора
        /// данных нет.</summary>
