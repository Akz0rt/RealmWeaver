    ...
    public static class WizardOps
    {
        ...
        public static List<string> DescribeClassChange(CharacterFile file, RulesData rules, string newClassId)
        {
            ...
            else
            {
                ...
                // снять её тихо: ApplyClassChange ниже считает по тем же двум функциям.
                //
                // ВЛАДЕНИЕ СЧИТАЕТСЯ ПО ОБОИМ ИСТОЧНИКАМ и на состояние ПОСЛЕ перехода: навык от
                // предыстории смену класса переживает, а с ним и компетентность на нём — Волшебник
                // с предысторией «Мудрец» держит компетентность в Магии, откуда бы он ни пришёл.
                //
                // О компетентности, которая теряет ВЛАДЕНИЕ, здесь не говорится: если навык уходит,
                // его потеря уже названа выше и вторая строка про то же самое была бы шумом; а если
                // владения не было и раньше (файл из чужих рук), то и терять нечего — такая строка
                // на листе не считалась ни разу.
                var alsoLost = new HashSet<string>(lostSkills.Select(l => l.Id));
                var bg = rules.Backgrounds.FirstOrDefault(b => b.Id == file.BackgroundId);
                var keeps = SheetMath.ProficientSkillIds(
                    file.SkillIds.Where(id => !alsoLost.Contains(id)), bg);
                foreach (var name in file.ExpertiseIds
                             .Where(id => keeps.Contains(id) && !next.AllowsExpertiseIn(id))
        ...
        public static void ApplyClassChange(CharacterFile file, RulesData rules, string newClassId)
        {
            ...
            // Два условия, а не одно: компетентность держится на ВЛАДЕНИИ навыком (а владение — это
            // выбранное у класса ПЛЮС данное предысторией, см. SheetMath.ProficientSkillIds) И на
            // разрешении класса брать её именно в этом навыке (ExpertiseChoices). У Волшебника,
            // например, Проницательность в списке навыков есть, а компетентности в ней класс не даёт
            // — и оставленную строку нечем было бы снять.
            //
            // Владение спрашивается ПОСЛЕ снятия навыков выше — то есть про уже новое состояние
            // файла. Навык от предыстории в нём остаётся, поэтому компетентность на нём смену класса
            // переживает, если новый класс её в этом навыке даёт.
            else
            {
                var proficient = SheetMath.ProficientSkillIds(file, rules);
                file.ExpertiseIds.RemoveAll(id => !proficient.Contains(id)
        ...
        ///
        /// КОМПЕТЕНТНОСТИ ЭТА ФУНКЦИЯ НЕ КАСАЕТСЯ ВОВСЕ, и это правка по существу, а не упрощение.
        /// Раньше она снимала компетентность со всего, чего нет в file.SkillIds, — и снимала её как
        /// раз с тех навыков, которые новая предыстория даёт даром: владение остаётся, а
        /// компетентность исчезала. Обратный случай — предыстория, которая навык БОЛЬШЕ НЕ даёт, —
        /// чинится сам собой при чтении: SheetMath.ExpertiseChosenIds пропускает только
        /// компетентность на навыке, которым персонаж владеет, так что осиротевшая перестаёт
        /// действовать, «чего не хватает» просит выбрать заново, а в файле она лежит безвредно —
        /// ровно как компетентность, отрезанная понижением уровня.
        ///
