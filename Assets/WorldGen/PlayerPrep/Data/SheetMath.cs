...
namespace WorldGen.PlayerPrep.Data
{
    ...
    public static partial class SheetMath
    {
        ...
        /// <summary>«1 навык», «2 навыка», «5 навыков» — слово после ЧИСЛА. Русское число требует
        /// трёх форм, и «1 навыка» в живом тексте выглядит опечаткой ровно так же, как «1 навыков».
        ///
        /// Исключение на 11–14 не косметическое: одиннадцать по последней цифре попало бы в «1
        /// навык». Считаем по двум последним цифрам, как и положено.</summary>
        public static string SkillsAfterNumber(int n)
        {
            int abs = System.Math.Abs(n), tens = abs % 100, ones = abs % 10;
            if (tens >= 11 && tens <= 14) return "навыков";
            if (ones == 1) return "навык";
            if (ones >= 2 && ones <= 4) return "навыка";
            return "навыков";
        }

        /// <summary>«в 1 навыке», «в 2 навыках» — форма после предлога «в». Отдельно от
        /// SkillsAfterNumber, потому что падеж другой: у именительного три формы, у предложного
        /// две, и одна функция на оба случая давала бы «в 2 навыка».</summary>
        public static string SkillsInNumber(int n)
        {
            int abs = System.Math.Abs(n), tens = abs % 100, ones = abs % 10;
            if (tens >= 11 && tens <= 14) return "навыках";
            return ones == 1 ? "навыке" : "навыках";
        }
        ...
        /// (Compute), и объяснение (ExplainAbility), и панель плана (LevelPlanView — показать, какая
        /// характеристика уже на потолке) спрашивают здесь. Сам план прокачки при записи ничего не
        /// считает: LevelChoiceOps.ChooseAsi кладёт обещанное целиком, а режет его этот счёт.
        ///
        ...
        ///
        /// ПОТОЛОК 20 ДЕРЖИТСЯ ЗДЕСЬ, И ТОЛЬКО ЗДЕСЬ, и это исправление двух находок подряд. Пока
        ...
        /// БАЗА в мастере. Проверять на записи бессмысленно — запись никогда не последняя. Обрезка при
        /// записи не только не работала, но и МОЛЧА СЪЕДАЛА выбор: «+2» превращалось в «+1» ещё до
        /// того, как о нём мог рассказать хоть кто-нибудь. Теперь в файл ложится обещанное целиком,
        /// урезает счёт, а называет потерю ExplainAbility.
        ///
