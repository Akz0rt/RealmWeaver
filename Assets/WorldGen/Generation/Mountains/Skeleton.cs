...
namespace WorldGen.Generation.Mountains
{
    ...
    /// в нарезку на звенья.
    ///
    /// Считается на ПОЛНОЙ сетке маски, без огрубления. Огрубление вдвое дало бы выигрыш в скорости,
    /// но сдвинуло бы все пороги, записанные в ячейках (зазор пузыря 5, радиус кластера 3.5, плечо
    /// направления 10): в клетках они остались бы прежними, а в мире стали бы вдвое больше. Цена
    /// утоньшения при этом не та, что кажется: список активных ячеек тает с каждым проходом, поэтому
    /// работа не «проходов × вся сетка», а порядка объёма массы, помноженного на её толщину.
    /// </summary>
