...
namespace WorldGen.Generation.Mountains
{
    ...
    public static class Skeleton
    {
        ...
        public static byte[] Thin(MountainMask mask)
        {
            ...
            // Первые два полупрохода идут ПО ВСЕЙ СЕТКЕ подряд, без списка живых ячеек. Список тут
            // был бы самой большой выкладкой во всём расчёте — у массива размером со страну это три
            // с половиной мегабайта, — а нужен он ровно два раза из сотен. Активными считаются
            // только внутренние ячейки: у краевой нет полного кольца соседей, а маска и так строится
            // с полем фона по краям (MountainMask.PadCells).
            ...
            while (changed && guard++ < ThinGuard)
            {
                ...
                for (int step = 0; step < 2; step++, sub++)
                {
                    ...
                    int span = active != null ? active.Count : (h - 2) * (w - 2);
                    for (int slot = 0; slot < span; slot++)
                    {
                        int idx;
                        if (active != null) idx = active[slot];
                        else { int row = slot / (w - 2); idx = (row + 1) * w + (slot - row * (w - 2)) + 1; }
                    ...
                    if (kill.Count > 0)
                    {
                        ...
                        {
                            // Гашение и разметка соседей — одним проходом, и «уже нулевая» служит
                            // отметкой «этот дубль уже разобран». Дубли в списке бывают: фронт их не
                            // отсеивает (см. ниже), и ячейку могли проверить дважды за полупроход.
                            // Разбирать дубль второй раз нельзя — иначе соседи попали бы в запачканные
                            // ещё по разу, и число повторов росло бы от полупрохода к полупроходу.
                            if (img[i] == 0) continue;
                            img[i] = 0;

                            // Сосед внутренней ячейки может лежать НА КРОМКЕ, а кромка — не наша: у
                            // неё нет полного кольца соседей, и сплошной обход её тоже никогда не
                            // трогал. Втянув краевую ячейку, фронт убил бы её и полез за её соседом
                            // за пределы сетки — ровно на этом первая редакция и упала. Кромка бывает
                            // не пустой, хотя маска и строится с полем в шесть ячеек: сглаживание
                            // (MountainMask.Smooth) раздувает массу и на мелком шаге достаёт до края.
                            int iy = i / w, ix = i - iy * w;
                            if (ix > 1 && ix < w - 2 && iy > 1 && iy < h - 2)
                            {
                                // Ячейка не у самой кромки — все восемь соседей заведомо внутренние.
                                for (int k = 0; k < 8; k++)
                                {
                                    int nb = i + offset[k];
                                    if (img[nb] != 0) dirtyNow.Add(nb);
                                }
                            }
                            else
                            {
                                for (int k = 0; k < 8; k++)
                                {
                                    int nx = ix + RingDx[k], ny = iy + RingDy[k];
                    ...
                    {
                        // Повторы во фронте НЕ отсеиваем намеренно. Отсев требовал бы отметки на
                        // каждую ячейку сетки — массив в мегабайт, который при счёте под кистью
                        // заводился бы десятки раз в секунду и утаскивал сборку мусора в старшее
                        // поколение, то есть возвращал бы ту самую заминку. А повторов у ячейки не
                        // больше, чем у неё соседей: каждый умерший сосед кладёт её ровно раз, и
                        // разбирается каждый умерший ровно раз (см. выше). Лишняя проверка стоит
                        // трёх десятков действий и ответа не меняет.
                        var build = active == frontA ? frontB : frontA;
                        build.Clear();
                        for (int i = 0; i < dirtyPrev.Count; i++)
                        {
                            int c = dirtyPrev[i];
                            if (img[c] != 0) build.Add(c);
                        }
