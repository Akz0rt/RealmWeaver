...
namespace WorldGen.Generation.Mountains
{
    ...
    public static class MountainInk
    {
        ...
        /// площадь/9 — это (площадь/R²)·100/9, то есть 11.1. Первая редакция стояла 74: я решил,
        /// что снимок сделан на фикстуре радиуса 26, и ошибся в семь раз — в приложении крошка вышла
        /// бы кляксой вместо зерна.
        /// </summary>
        ...
        /// ровно браузерные: sz от 0.9 до 2.6 при радиусе 10, то есть от 0.09·R до 0.26·R. Метка
        /// кладётся ПРЯМОУГОЛЬНИКОМ sz×0.7sz, тоже как там: треугольник той же ширины кроет вдвое
        /// меньше, и первая редакция переноса вышла вдвое светлее оригинала, а вблизи он ещё и
        /// читался треугольником, а не зерном.
        /// </summary>
        public static float MarkSize(float radius, float u)
        {
            if (u < 0f) u = 0f; else if (u > 1f) u = 1f;
            return radius * (0.09f + u * 0.17f);
        }
