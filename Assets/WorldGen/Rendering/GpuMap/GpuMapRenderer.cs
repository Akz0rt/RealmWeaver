...
namespace WorldGen.Rendering.GpuMap
{
    ...
    public class GpuMapRenderer : MonoBehaviour
    {
        ...
        // Поле дистанции считается в пониженном разрешении (гладкое, билинейное). Было 1/4, стало
        // 1/2 ради нарисованных рек: их тело — считанные пиксели поперёк, и на сетке через 4
        // пикселя оно ловилось отсчёт через отсчёт, отчего ореол и песчаная кромка шли вдоль реки
        // клочьями. Вчетверо больше отсчётов, но это по-прежнему четверть карты и считается
        // только на отпускании кисти.
        const int CoastDownscale = 2;
