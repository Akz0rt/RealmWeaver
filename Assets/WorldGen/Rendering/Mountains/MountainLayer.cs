...
namespace WorldGen.Rendering.Mountains
{
    ...
    public class MountainLayer : MonoBehaviour
    {
        ...
        /// результат).
        ///
        /// ОТБРАСЫВАНИЯ ОТВЕТОВ ПО НОМЕРУ ПОКОЛЕНИЯ ЗДЕСЬ БОЛЬШЕ НЕТ, и это не уборка ради уборки.
        /// Номер завели, когда правка запускала свою задачу и ответы могли прийти не по порядку. С
        /// тех пор счёт стал один за раз (см. ниже), не по порядку прийти нечему — а номер остался и
        /// начал вредить: ЛЮБАЯ правка, пришедшая пока считали, делала ответ «чужим», и он
        /// выбрасывался. Пока ДМ ведёт кисть, правки идут непрерывно, значит выбрасывалось ВСЁ, и
        /// рисунок появлялся только когда рука замирала. Ровно это и мешало показу под кистью.
        /// </summary>
        public void Rebuild()
        {
            EnsureContainer();
            // Отложенный пересчёт (RebuildSoon) больше не нужен — считаем прямо сейчас. Иначе после
            // мазка приходил бы ещё один счёт того же самого, уже посчитанного.
            rebuildAt = -1f;
        ...
        void Update()
        {
            ...
            // Ответ берём ВСЕГДА, даже если правки уже успели уйти вперёд. Он посчитан по снимку,
            // сделанному честно, и отстать может самое большее на одну правку — а следом за ним тут
            // же стартует счёт по свежему снимку (queued ниже). Показ, отставший на мазок-другой,
            // глазу незаметен; показ, которого нет вовсе, — это и была болезнь.
            //
            // И ещё: именно Apply кладёт в кэш посчитанные массивы. Выброшенный ответ уносил кэш с
            // собой, и следующий счёт начинал с нуля — то есть выброс стоил не только показа, но и
            // вчетверо-впятеро большей цены каждого следующего счёта.
            if (!finished.IsCanceled) Apply(finished.Result);
