...
namespace WorldGen.Persistence
{
    ...
    public static class ProjectSerializer
    {
        public const int CurrentFormatVersion = 18;  // 18: у карты появились реки, нарисованные кистью.
                                                     ...
                                                     //
                                                     // 16 и 17 писались ПАРАЛЛЕЛЬНО, в двух ветках сразу.
                                                     // Арка персонажей видела в своём дереве ещё 15 и взяла
                                                     // 17 намеренно, оставив 16 арке «бумажных линий»: наивный
                                                     // +1 дал бы обеим один номер и разошёлся бы при слиянии.
                                                     // Здесь они уже сведены, и нумерация снова сплошная.
                                                     //
                                                     // 16: мазок рисунка стал цепочкой точек, у листа появился тон. Миграции нет: старый рисунок
                                                     // это PNG без мазков, что и означает «унаследованный слой». Бумп документирующий — старая
                                                     // сборка предупредит, вместо того чтобы молча потерять мазки.
                                                     //
