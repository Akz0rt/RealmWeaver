    ...
    public static class Mutants
    {
        public static void Run()
        {
            var cases = new List<(string name, string what, Action run)>
            {
                ...
                ("MutOnFieldNoLowerBound", "SettlementVolumeRendererPlacement.OnFieldCell drops only its lower x-bound, so a negative normalized X is no longer refused while the upper bound and both y-bounds stay intact (review finding I1/M1 fix round)",
