    ...
    public static class Mutants
    {
        public static void Run()
        {
            var cases = new List<(string name, string what, Action run)>
            {
                ...
                ("MutNoInsideFilter",  "SettlementGenerator.PlaceBuildings drops the !wall.Contains(cx, cy) term from the skip condition",
                    () => new WorldGen.MutantTests.MutNoInsideFilterSelfTests().SelfTestBuildings()),
                ("MutNoWallClearance", "SettlementGenerator.PlaceBuildings drops the wall.DistanceToEdge(...) < half term from the skip condition",
                ...
                ("MutSpacingNoParity", "SettlementGenerator.PlaceBuildings' checkerboard parity split degenerated — every candidate lands in `even`, so 4-adjacent cells are taken without a neighbour test",
                    () => new WorldGen.MutantTests.MutSpacingNoParitySelfTests().SelfTestBuildingSpacing()),
                ("MutSpacingNoAdjacencyCheck", "SettlementGenerator.PlaceBuildings' odd-tier NoNeighbour guard neutered (top-up ignores neighbours)",
                    () => new WorldGen.MutantTests.MutSpacingNoAdjacencyCheckSelfTests().SelfTestBuildingSpacing()),
                ("MutRadiusLinear",    "SettlementGenerator.WallRadiusFor reverted to the old linear law (0.16f + 0.0045f * buildingCount)",
                    () => new WorldGen.MutantTests.MutRadiusLinearSelfTests().SelfTestBuildingSpacing()),
