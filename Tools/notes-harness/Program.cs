...
static class Program
{
    ...
    //
    // THE GUARANTEE HAS THREE PARTS, and for most of this arc only the first was true — which is exactly the
    // "green by absence" shape the reflection exists to prevent, hiding inside the thing preventing it:
    //   1. every *SelfTests TYPE in the two namespaces is discovered (this method);
    //   2. every FILE in the two source directories is compiled at all — sync.ps1 globs `*.cs`, and used to
    ...
    static int ReportUninvokedSelfTests(List<Type> suites, List<(Type Suite, MethodInfo Method)> invoked)
    {
        ...
        if (count > 0)
            Console.WriteLine($"NOTES SELF-TESTS: {count} method(s) named SelfTest* were not invoked — failing rather than reporting a partial count.");
        return count;
    }
}
